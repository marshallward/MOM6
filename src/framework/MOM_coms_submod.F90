...
!! forced inlining is required. NaN/overflow are reported via flags rather than
!! the module-level error logicals, so the routine is side-effect free.
pure subroutine efp_decompose_arr(r, e, rmag, is_nan, is_ovf)
  real,                intent(in)  :: r      !< The real number being decomposed [a]
  ...
  ! I think just 0 is ok here?  TODO
  e(:) = 0_int64
  rmag = 0.0 ; is_nan = 0 ; is_ovf = 0
  ...
  do n=1,ni
    ...
    e(n) = sgn*ival
  enddo
end subroutine efp_decompose_arr
...
module subroutine increment_ints_faster(int_sum, r, max_mag_term)
  ...
  !call efp_decompose_arr(r, e, rmag, is_nan, is_ovf)
...
module subroutine increment_ints_2d(array, is, ie, js, je, descale, ints_sum, max_mag_term)
  ...
  do concurrent (j=js:je, i=is:ie) &
      DO_LOCALITY(local(r, e1, e2, e3, e4, e5, e6, rmag, lnan, lovf)) &
      !DO_LOCALITY(local(r, e, rmag, lnan, lovf)) &
      DO_LOCALITY(reduce(+: ints_sum) reduce(max: mmag, inan, iovf))
  !!$omp target loop private(r, e, rmag, lnan, lovf) &
  !!$omp   reduction(+: s) reduction(max: mmag, inan, iovf)
  !do j=js,je ; do i=is,ie
    r = descale*array(i,j)

    call efp_decompose(r, e1, e2, e3, e4, e5, e6, rmag, lnan, lovf)
    !call efp_decompose_arr(r, e, rmag, lnan, lovf)
    ...
    !ints_sum(:) = ints_sum(:) + e(:)
    ints_sum(1) = ints_sum(1) + e1
    ints_sum(2) = ints_sum(2) + e2
    ints_sum(3) = ints_sum(3) + e3
    ints_sum(4) = ints_sum(4) + e4
    ints_sum(5) = ints_sum(5) + e5
    ints_sum(6) = ints_sum(6) + e6
  enddo
