  ...
  do m=1,2
    ...
    do concurrent (j=jstart:Jeq)
      ...
      if (use_BBL_EOS .or. CS%body_force_drag .or. .not.CS%linear_drag) then
        ...
        do concurrent (i=is:ie) DO_LOCALITY(local(cdrag_sqrt)) DO_LOCALITY(local_init(cdrag_sqrt_H))
          if (CS%bottomdragmap) then
            if (m==1) then
              cdrag_sqrt = sqrt(CS%cdrag_u(i,j))
            else
    ...
    !$omp   I_hwtot, h_bbl_fr, root, cdrag) firstprivate(cdrag_L_to_H, cdrag_RL_to_H)
