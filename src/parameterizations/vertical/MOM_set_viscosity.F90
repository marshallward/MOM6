...
!    if (CS%linear_drag) ustar(:) = cdrag_sqrt_H*CS%drag_bg_vel
  ...
  do jstart=js,je,TILE_SIZE_Y ; do istart=Isq,Ieq,TILE_SIZE_X  ! u-point loop
    ...
    if (do_any_shelf) then
      ...
      if (use_EOS) then
        do j=jstart,jend
        jj=j-jstart+1
        call calculate_density_derivs(T_EOS(:,jj), S_EOS(:,jj), forces%p_surf(istart:iend,j), &
  ...
  !$omp   S_EOS, dz, mask_u, mask_v, h_at_vel, dz_at_vel, k_massive, u2_bg, do_i, tv, tv%T, tv%S, &
  !$omp   tv%p_surf, CS)
